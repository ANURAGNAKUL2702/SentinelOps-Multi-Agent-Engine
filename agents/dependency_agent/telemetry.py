...
Adapted from metrics_agent.telemetry — same patterns,
dependency-domain layer naming (graph_build, trace_analysis,
impact_calculation, classification, validation, pipeline_total).
"""
...
def get_logger(name: str = "dependency_agent") -> logging.Logger:
    ...
    Example::
        ...
        log = get_logger("dependency_agent.graph_builder")
        log.info("Graph built", extra={"correlation_id": cid})
    """
...
class TelemetryCollector:
    """Collects metrics for all layers of the dependency agent pipeline.
    ...
    Example::
        ...
        with tel.measure("graph_build"):
            pass
        print(tel.snapshot())
    """

    def __init__(self) -> None:
        self._log = get_logger("dependency_agent.telemetry")

        # ── latency histograms per layer ────────────────────────
        self.latency: Dict[str, _Histogram] = {
            "graph_build": _Histogram(),
            "trace_analysis": _Histogram(),
            "impact_calculation": _Histogram(),
            "bottleneck_detection": _Histogram(),
    ...
    def measure(
        ...
        Args:
            layer: One of 'graph_build', 'trace_analysis',
                   'impact_calculation', 'bottleneck_detection',
                   'classification', 'validation', 'pipeline_total'.
            correlation_id: Request correlation ID.

        Example::

            with tel.measure("graph_build", cid):
